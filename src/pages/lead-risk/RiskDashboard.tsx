...
export default function RiskDashboard() {
  ...
  const totalEntities = mockRiskProfiles.length;
  const criticalEntities = mockRiskProfiles.filter(p => p.riskLevel === 'critical').length;
  const highEntities = mockRiskProfiles.filter(p => p.riskLevel === 'high').length;
  const watchlistedEntities = mockRiskProfiles.filter(p => p.isWatchlisted).length;
  const alertEntities = mockRiskProfiles.filter(p => p.hasActiveAlert).length;
  const inProgressLeads = metrics.inProgress;
  const overdueLeads = metrics.overdue;
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        ...
        <button className={styles.createButton} onClick={() => setIsFormModalOpen(true)}>
          <Plus size={20} />
          Thêm rủi ro
        </button>
      </div>
      ...
      <div className={styles.overviewGrid}>
        ...
        >
          ...
          <div className={styles.metricValue}>{totalEntities}</div>
          <div className={styles.metricDetail}>
            <span className={styles.metricDetailLabel}>Cửa hàng:</span>
            <span className={styles.metricDetailValue}>
              {mockRiskProfiles.filter(p => p.entityType === 'store').length}
            </span>
          </div>
        </div>
        ...
        >
          ...
          <div className={styles.metricValue}>{criticalEntities}</div>
          <div className={styles.metricDetail}>
        ...
        >
          ...
          <div className={styles.metricValue}>{highEntities}</div>
          <div className={styles.metricDetail}>
        ...
        >
          ...
          <div className={styles.metricValue}>{watchlistedEntities}</div>
          <div className={styles.metricDetail}>
        ...
        >
          ...
          <div className={styles.metricValue}>{inProgressLeads}</div>
          <div className={styles.metricDetail}>
      ...
      <div className={styles.filterRow}>
        <MultiSelectDropdown
          label="Mức rủi ro"
          options={[
            { value: 'critical', label: 'Nghiêm trọng', count: mockRiskProfiles.filter(p => p.riskLevel === 'critical').length },
            { value: 'high', label: 'Cao', count: mockRiskProfiles.filter(p => p.riskLevel === 'high').length },
            { value: 'medium', label: 'Trung bình', count: mockRiskProfiles.filter(p => p.riskLevel === 'medium').length },
            { value: 'low', label: 'Thấp', count: mockRiskProfiles.filter(p => p.riskLevel === 'low').length },
          ]}
        ...
        <MultiSelectDropdown
          label="Loại cơ sở"
          options={[
            { value: 'store', label: 'Cửa hàng', count: mockRiskProfiles.filter(p => p.entityType === 'store').length },
            { value: 'zone', label: 'Khu vực', count: mockRiskProfiles.filter(p => p.entityType === 'zone').length },
          ]}
        ...
        <MultiSelectDropdown
          label="Xu hướng"
          options={[
            { value: 'increasing', label: 'Tăng', count: mockRiskProfiles.filter(p => p.trendDirection === 'increasing').length },
            { value: 'stable', label: 'Ổn định', count: mockRiskProfiles.filter(p => p.trendDirection === 'stable').length },
            { value: 'decreasing', label: 'Giảm', count: mockRiskProfiles.filter(p => p.trendDirection === 'decreasing').length },
          ]}
        ...
        <MultiSelectDropdown
          label="Theo dõi"
          options={[
            { value: 'watched', label: 'Đang theo dõi', count: watchlistedEntities },
            { value: 'not_watched', label: 'Chưa theo dõi', count: totalEntities - watchlistedEntities },
          ]}
      ...
      <div className={styles.riskTable}>
        <div className={styles.tableHeader}>
          ...
          <div className={styles.headerCell} style={{ flex: '0 0 120px', textAlign: 'center' }}>Risk Score</div>
          <div className={styles.headerCell} style={{ flex: '0 0 110px', textAlign: 'center' }}>Tổng Leads</div>
          ...
          {filteredProfiles.length > 0 ? (
            filteredProfiles.map((profile) => (
              ...
              >
                ...
                <div className={styles.riskCell} style={{ flex: '0 0 140px' }} onClick={(e) => e.stopPropagation()}>
      ...
      {filteredProfiles.length > 0 && (
        <div className={styles.pagination}>
          <div className={styles.paginationInfo}>
            Hiển thị {filteredProfiles.length} / {mockRiskProfiles.length} cơ sở
          </div>
