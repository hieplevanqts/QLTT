...
export function IDCardUploadDialog({
  ...
  const validateFile = (file: File): string | null => {
    ...
    if (!allowedTypes.includes(file.type)) {
      return 'Chỉ chấp nhận file JPG, PNG, WebP hoặc PDF';
    }
  ...
  const handleOcrExtraction = async (front: File, back: File) => {
    ...
    try {
      // Import dynamically to avoid circular dependencies if any, or just consistent
      const { callOcrCccdApi } = await import('@/utils/api/ocrApi');
      ...
      if (result.success && result.data) {
        ...
        setTimeout(() => setExtractionComplete(false), 5000);
      } else {
        // Show error but don't clear form
        toast.error(result.message || 'Không thể nhận diện CCCD');
      }
    } catch (err) {
      console.error('OCR failed:', err);
      toast.error('Lỗi khi gọi dịch vụ OCR');
    } finally {
      setIsExtracting(false);
    }
  };
  ...
  const handleFrontFileChange = async (file: File) => {
    ...
    // Check backFile from state
    if (backFile) {
      handleOcrExtraction(file, backFile);
    }
  };
  ...
  const handleBackFileChange = async (file: File) => {
    ...
    // Auto-extract if both files are present
    // Check frontFile from state
    if (frontFile) {
      handleOcrExtraction(frontFile, file);
    }
  };
