...
import styles from './KeHoachTacNghiepTaskBoard.module.css';
...
const TASK_COLUMNS: { id: TaskStatus; label: string; color: string; enabled: boolean; dropEnabled: boolean }[] = [
  ...
  { id: 'closed', label: 'Đã đóng', color: 'rgba(102, 112, 133, 1)', enabled: false, dropEnabled: false }, // Chỉ admin mới set được
];
  ...
  return (
    ...
    >
      <div className={styles.taskCardHeader}>
        <KeHoachTacNghiepStatusBadge type="priority" value={task.priority} size="sm" />
      ...
      <div className={styles.taskCardFooter}>
        <KeHoachTacNghiepStatusBadge type="sla" value={task.sla.status} size="sm" />
...
function TaskColumn({ column, tasks, onTaskClick, onDropTask }: TaskColumnProps) {
  const [{ isOver, canDrop }, drop] = useDrop({
    accept: 'TASK',
    canDrop: () => column.dropEnabled, // Ch�� cho phép drop nếu cột enabled
...
export function KeHoachTacNghiepTaskBoard() {
  ...
  // Find plan first for early return check
  ...
  // Create unique assignees list using Map to ensure no duplicates
