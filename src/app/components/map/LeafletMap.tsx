  ...
  const updateMarkers = useCallback(() => {
    ...
    if (showWardBoundaries && !showMerchants) {
      ...
      if (isLoadingDepartmentAreas) {
        ...
        console.log('🗺️ LeafletMap: Rendering department markers', {
        ...
        if (!departmentMapData.areas || departmentMapData.areas.length === 0) {
          console.warn('⚠️ LeafletMap: No areas in departmentMapData');
          return;
        }
        ...
        departmentMapData.areas.forEach((area, index) => {
          if (area.coordinates.center) {
            ...
            if (index < 3) { // Log first 3 areas without center
              console.warn('⚠️ LeafletMap: Area missing center coordinates:', {
                wardId: area.wardId,
                provinceId: area.provinceId,
        ...
        console.log('🗺️ LeafletMap: Ward markers map created:', {
        ...
        wardMarkersMap.forEach((wardData) => {
          ...
          wardMarker.addTo(mapInstanceRef.current);
          markersRef.current.push(wardMarker);
        });
        
        console.log('✅ LeafletMap: Rendered', wardMarkersMap.size, 'ward markers from', departmentMapData.areas.length, 'areas');
      }
